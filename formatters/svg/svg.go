...
func (f *Formatter) writeSVG(w io.Writer, style *zdpgo_pygments.Style, tokens []zdpgo_pygments.Token) { // nolint: gocyclo
	...
	fmt.Fprintf(w, "<rect width=\"100%%\" height=\"100%%\" fill=\"%s\"/>\n", style.Get(zdpgo_pygments.Background).Background.String())
	fmt.Fprintf(w, "<g font-family=\"%s\" font-size=\"14px\" fill=\"%s\">\n", f.fontFamily, style.Get(zdpgo_pygments.Text).Colour.String())
...
func maxLineWidth(lines [][]zdpgo_pygments.Token) int {
	maxWidth := 0
...
// adding the token.
func (f *Formatter) writeTokenBackgrounds(w io.Writer, lines [][]zdpgo_pygments.Token, style *zdpgo_pygments.Style) {
	for index, tokens := range lines {
		lineLength := 0
		for _, token := range tokens {
			length := len(strings.ReplaceAll(token.String(), `	`, "    "))
			tokenBackground := style.Get(token.Type).Background
			if tokenBackground.IsSet() && tokenBackground != style.Get(zdpgo_pygments.Background).Background {
...
func (f *Formatter) styleAttr(styles map[zdpgo_pygments.TokenType]string, tt zdpgo_pygments.TokenType) string {
...
func (f *Formatter) styleToSVG(style *zdpgo_pygments.Style) map[zdpgo_pygments.TokenType]string {
	converted := map[zdpgo_pygments.TokenType]string{}
...
// StyleEntryToSVG converts a zdpgo_pygments.StyleEntry to SVG attributes.
func StyleEntryToSVG(e zdpgo_pygments.StyleEntry) string {
	var styles []string
